...
$battlebot = $install_path.'/battlebot-pthread';
$pug_queue = array();
...
// echo  "\n";
    ...
    if(in_array($data->channel,$channel)) {
      if($pug_queue) { 
	...
	// Check if player joins the invite channel
      $p_channel = explode(":",$data->channel);
      $g_channel = explode("-",$p_channel[2]);
      $g_team = substr($g_channel[3], 4); // remove team prefix

      if(isset($pug_queue[$g_channel[0]][$g_channel[1]][$g_channel[2]]['inv'.$g_channel[3]])) {
        if(in_array($data->nick,$pug_queue[$g_channel[0]][$g_channel[1]][$g_channel[2]]['inv'.$g_channel[3]])) {
          // Check if team is full in the meantime
          ...
          if(count($pug_queue[$g_channel[0]][$g_channel[1]][$g_channel[2]][$g_channel[3]]) < $team_size[$g_team]) {
            // Add player to team!
            // Check if all teams are full now!
            $pug_queue[$g_channel[0]][$g_channel[1]][$g_channel[2]][$g_channel[3]][] = $data->nick;
            ...
            foreach($team_size as $key => $value)
            {
              ...
              $players = $players + count($pug_queue[$g_channel[0]][$g_channel[1]][$g_channel[2]]['team'.$key]);
            }
          ...
          $pug_queue[$g_channel[0]][$g_channel[1]][$g_channel[2]]['inv'.$g_channel[3]] = array_values(array_diff($pug_queue[$g_channel[0]][$g_channel[1]][$g_channel[2]]['inv'.$g_channel[3]],array($data->nick)));          
        }
      }
    }
  }
        ...
        foreach($pug_mode as $key3 => $pug_skill) {
          foreach($pug_skill as $key4 => $pug_team) {
            if(is_array($pug_game[$key2][$key3][$key4])) {
              if(in_array($player, $pug_game[$key2][$key3][$key4])) {
...
// Default: $bb_shortmsg = 1;
    ...
    foreach($team_size as $key => $value)
    {
      $team[$key] = implode(",", $pug_queue[$pug_game][$pug_mode][$pug_skill]['team'.$key]);
    ...
    if($bb_shortmsg == 0) {
      ...
      $irc->message(SMARTIRC_TYPE_CHANNEL, $channel[0], ''.$prefix_msg.' '.$info_msg.' '.$suffix_msg.'');        
      $irc->message(SMARTIRC_TYPE_CHANNEL, $channel[$pug_game], ''.$prefix_msg.' '.$info_msg.' '.$suffix_msg.'');        
    } else {
      $irc->message(SMARTIRC_TYPE_CHANNEL, $channel[$pug_game], ''.$prefix_msg.' '.$info_msg.' '.$suffix_msg.'');        
    }
  }
...
// echo "\n";
    ...
    if($return[0] == 'owner') {
      ...
      // Check for new owner
      foreach($team_size as $key => $value) {
        if(array_key_exists('0', $pug_queue[$return['game']][$return['mode']][$return['skill']]['team'.$key])) {
          $pug_queue[$return['game']][$return['mode']][$return['skill']]['owner'] = $pug_queue[$return['game']][$return['mode']][$return['skill']]['team'.$key][0];
          break;
        }
      }
      ...
      if($pug_queue[$return['game']][$return['mode']][$return['skill']]['owner'] == $data->nick) {
        ...
        $irc->message(SMARTIRC_TYPE_CHANNEL, $channel[0], 'PUG Removed: '.$return['game'].' '.$return['mode'].' '.$return['skill'].'');
        $irc->message(SMARTIRC_TYPE_CHANNEL, $channel[$return['game']], 'PUG Removed: '.$return['game'].' '.$return['mode'].' '.$return['skill'].'');
      } else {
...
/*command: !info*/
  function pug_info($irc, $data) {
    global $pug_queue, $extended, $channel, $ch_suffix;
...
//  $pugstring = trim(array_pop(explode('!info', $data->message)));
    ...
    if (!isset($pugdata[1])) {
      ...
      if(preg_match('/^[0-9]+v[0-9]+(v[0-9]+)*$/', $pugdata[1])) {
        ...
        if($data->channel == $channel[0] || strpos($data->channel,$pugdata[1])) {
...
/*command: !create*/
  function pug_create($irc, $data) {
    global $battlebot, $pug_queue, $channel, $ch_suffix;
...
*/
    ...
    if(isset($pug_queue[$p_game][$p_mode][$p_skill])) { 
      ...
      if(!in_array($p_game,$p_gamearray)) {
        ...
        if($data->channel != $channel[0] && !strpos($data->channel,$p_game)) {
          $irc->message(SMARTIRC_TYPE_NOTICE, $data->nick, 'Please join '.$channel[$p_game].'');
          return;
        } 
		...
		// <--   
            ...
            foreach($team_size as $key => $value) 
            {
			  ...
			  $irc->mode(${$p_game}[$p_mode][$p_skill]['irc']['team'.$teamkey], '+i');
			  $irc->setTopic($value,'IDLE FOR MATCH: '.$p_game.' '.$p_mode.''); 

			  ${$p_game}[$p_mode][$p_skill]['team'.$teamkey] = array();
			  ${$p_game}[$p_mode][$p_skill]['invteam'.$teamkey] = array();
			  ${$p_game}[$p_mode][$p_skill]['kickteam'.$teamkey] = array();
			}

			${$p_game}[$p_mode][$p_skill]['invteam1'][] = $data->nick;
...
/*command: !join*/
  ...
  function pug_join($irc, $data) {
    global $pug_queue, $channel, $ch_suffix, $bb_shortcmd;
    $pug_queue_list = $this->pug_queue_list($pug_queue);
...
// -->		echo $p_game;
        ...
        if(strpos($data->channel,$pugdata[0]) || $data->channel == $channel[0] ) {
          if(isset($pugdata[0])) { $p_game   = strtolower($pugdata[0]); }
          if(isset($pugdata[1])) { $p_mode   = strtolower($pugdata[1]); }
          if(isset($pugdata[2])) { $p_skill  = strtolower($pugdata[2]); }
      ...
      // Check if player isn't kicked
	    if(!in_array($data->nick, $pug_queue[$p_game][$p_mode][$p_skill]['kickteam'.$p_team])){ 
		// Feel free to join 
          if(count($pug_queue[$p_game][$p_mode][$p_skill]['team'.$p_team]) < $team_size[$p_team]) {
            // Put player in place holder until he actually joins the channel!
            $pug_queue[$p_game][$p_mode][$p_skill]['invteam'.$p_team][] = $data->nick;              
...
/*command: !need*/
  function pug_need($irc, $data) {
    global $pug_queue, $channel, $ch_suffix;
...
// echo "\n";
    ...
    $irc->message(SMARTIRC_TYPE_CHANNEL, $channel[$return['game']], 'There are still '.$players_needed.' players needed for '.$return['game'].' '.$return['mode'].' '.$return['skill'].'!');             
...
// Prevent new vote (in THIS PUG) from being called untill current vote is finished!!! (we have pugID and votecaller teamID)
// Prevent new vote (in THIS PUG) from being called by the same person untill cooldown period has passed (track his nameID)
// Determine votetype:
// - Kickvote
// - Mapvote
// - Startvote

    $cmd = "H:/php/php.exe H:/ircservers/ircbots/battlebot-pthread/timewrapper.php";
...
// -->
  ...
  {
    if($fn == "." || $fn == ".." || $fn == "default") { continue; } 
    $channel[$fn] = '#'.$fn.''.$ch_suffix.''; 	// REMOVE .PUG AND PUT INTO ARRAY
  }
  closedir($fh);
// -->
$key = null;
...
// $irc->registerActionhandler(SMARTIRC_TYPE_CHANNEL, '!vote.*', $bot, 'pug_vote');				// Players (Check vote command)
